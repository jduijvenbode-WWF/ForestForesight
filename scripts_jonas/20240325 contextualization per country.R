...
countrynames=c("Laos","Colombia","Gabon","Sarawak","Bolivia","Madre de Dios","Peru","Kalimantan","IDN")
isos=c("LAO","COL","GAB","MYS","BOL","PER","PER")
...
for(x in seq(length(countrynames))){
  ...
  if(country=="Borneo"){shape=disagg(shape)[133]}
  if(country=="Madre de Dios"){shape=aggregate(gadm_s[which(gadm_s$"NAME_1" == "Madre de Dios"),])}
  b <- ForestForesight::train_predict_raster(shape = shape,prediction_date = proc_date,train_start = train_start,train_end=train_end,ff_folder = "C:/data/storage",verbose = T)
