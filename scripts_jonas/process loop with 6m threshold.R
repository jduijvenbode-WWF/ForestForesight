...
files=files[-which(!file.exists(m6files))]
m6files=m6files[-which(!file.exists(m6files))]
files=files[-which(!file.exists(gtfiles))]
m6files=m6files[-which(!file.exists(gtfiles))]
gtfiles=gtfiles[-which(!file.exists(gtfiles))]
alldat=data.frame()
for(i in seq(length(files))){
  ...
  ff_analyze(predictions = pred>threshold,groundtruth = gt,return_polygons = F,tile=basename(dirname(files[i])),date=substr(basename(files[i]),10,19),csvfile = "D:/ff-dev/newres3.csv")
