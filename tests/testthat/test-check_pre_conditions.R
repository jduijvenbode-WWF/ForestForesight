...
test_that("check_pre_conditions works as expected", {
  ...
  expect_error(
    check_pre_conditions(NA, country, shape, tiles, shrink),
    "No dates were given"
  )

  # Test missing dates with NULL
  expect_error(
    check_pre_conditions(NULL, country, shape, tiles, shrink),
    "No dates were given"
  )

  # Test missing dates with an empty string
  expect_error(
    check_pre_conditions("", country, shape, tiles, shrink),
    "No dates were given"
  )

  # Test missing tiles, country, and shape
  expect_error(
    check_pre_conditions(dates, NULL, NULL, NULL, shrink),
    "Unknown what to process since no tiles, country, or shape were given"
  )

  # Test shape is not SpatVector
  expect_error(
    check_pre_conditions(dates, country, "invalid_shape", tiles, shrink),
