...
ff_analyze <- function(predictions, groundtruth, forestmask = NULL, csvfile = NULL, country = NULL,
  ...
  if (!is.null(country) & ("iso3" %in% names(pols))) {
  ...
  if (verbose) {
    ...
    ff_cat("F0.5 score is:", 1.25 * pr * re / (0.25 * pr + re), verbose = verbose)
  }
  ...
  if (addWKT) {
    pol_df <- as.data.frame(pols, geom = "wkt")
  } else {
    pol_df <- as.data.frame(pols)
  }
  if (!is.null(csvfile)) {
    if (append & file.exists(csvfile)) {
      ...
      pastdata$X <- NULL
      write.csv(rbind(pastdata, pol_df), csvfile)
    } else {
