...
data_quality <- function(folder_path) {
  ...
  summary_by_feature=function(dataframe,feature){
    ...
    return(list("feature"=feature,
                "mindate"=min(as.Date(seldf$dates)),
                "maxdate"=max(as.Date(seldf$dates)),
                ...
                "crsname"=if(length(unique(seldf$crsname))==1){"equal"}else{seldf$crsname[1]},
                "crscode"=if(length(unique(seldf$crscode))==1){"equal"}else{seldf$crscode[1]},
                "mean"=mean(seldf$mean),
                "max"=max(seldf$max),
  ...
  allvals=allvals[,c(12,13,seq(11))]
