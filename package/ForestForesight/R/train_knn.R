...
train_model <- function(train_data, valid_data, save_model = FALSE) {
  ...
  reduce_max_cores = 2
  cl <- makeCluster(max_cores - reduce_max_cores)
  registerDoParallel(cl)
  cat(sprintf("Using %d CPU cores for parallel processing\n", max_cores - reduce_max_cores))
  ...
  # Add cyclic transformations for the month
  #if ("month" %in% colnames(x_train)) {
  #  x_train$sine_month <- sin(2 * pi * x_train$month / 12)
  #  x_train$cosine_month <- cos(2 * pi * x_train$month / 12)
  #  x_validation$sine_month <- sin(2 * pi * x_validation$month / 12)
  #  x_validation$cosine_month <- cos(2 * pi * x_validation$month / 12)

    # Now remove the original 'month' column if no longer needed
  #  x_train$month <- NULL
  #  x_validation$month <- NULL
  #}
  ...
  ctrl <- trainControl(method = "repeatedcv", number = 5, repeats = 3, allowParallel = TRUE,
                       ...
                       summaryFunction = customPrSummary)


  cat("Training model...\n")
  old_time <- Sys.time()
  model <- train(x = x_train, y = y_train, method = "knn",
                 trControl = ctrl, tuneGrid=expand.grid(k=c(5,6,7,8,9)), metric="F0.5")
...
# train_data = ff_prep("D:/ff-dev/results/preprocessed/", tiles = "10S_060W", fltr_features="initialforestcover", fltr_condition='>0', validation_sample=0.1, sample_size=0.02, label_threshold=1, start="2021-06-01", end="2022-05-01")
# valid_data = ff_prep("D:/ff-dev/results/preprocessed/", tiles = "10S_060W", fltr_features="initialforestcover", fltr_condition='>0', validation_sample=0.99, sample_size=0.01, label_threshold=1, start="2022-12-01", end="2023-05-01")
